...
args.imbalance_coeff = 0
args.num_trials = 10
# args.log_root = '../logs0107'
args.upset_margin_coeff = 0
args.trainable_alpha = False
args.cluster_rank_baseline = 'syncRank'
args.lr = 0.01
args.hidden = 8
args.F = 2
args.K = 20
...
def evalutaion(logstr, score, A_torch, label_np, val_index, test_index, SavePred, save_path, split, identifier_str):
    ...
    upset = calculate_upsets(A_torch, torch.FloatTensor(-pred_label.reshape(pred_label.shape[0], 1)).to(args.device), style='simple').detach().item()
    upset_full = [upset, score_upset.detach().item()]
    if SavePred:
    ...
    logstr += 'upset simple:,{:.6f},upset ratio:,{:.6f},'.format(upset, score_upset.detach().item())
...
class Trainer(object):
    ...
    def train(self, model_name, normalization='plain', threshold='sort'):
        ...
        if model_name not in ['DIGRAC', 'ib']:
            ...
            upset_full = np.zeros([NUM_GNN_VARIANTS, self.splits, 2])
            upset_full[:] = np.nan
            upset_full_latest = upset_full.copy()
            ...
            for split in range(self.splits):
                ...
                trainable_alpha=self.args.trainable_alpha, initial_score=score_torch, prob_dim=self.num_clusters, sigma=self.args.sigma).to(self.args.device)
                    model_path = os.path.join(self.log_dir, 'DIGRAC_plain_sort_model'+str(split)+'.t7')
    ...
    def non_nn(self, model_name):
        ...
        kendalltau_full[:] = np.nan
        upset_full = np.zeros([self.splits, 2])
        upset_full[:] = np.nan
        A = scipy_sparse_to_torch_sparse(self.A).to(self.args.device)
        ...
        for split in range(self.splits):
            ...
            if model_name not in ['mvr']:
                ...
                if model_name not in ['syncRank']:
                    if upset1.detach().item() > upset2.detach().item():
                        score_upset = upset2.detach().item()
                        score = -score
                    else:
                        score_upset = upset1.detach().item()
            ...
            upset = calculate_upsets(self.A_torch, torch.FloatTensor(-pred_label.reshape(pred_label.shape[0], 1)).to(self.args.device), style='simple').detach().item()
            upset_full[split] = [upset, score_upset]
            logstr += 'upset simple:,{:.6f},upset score:,{:.6f},'.format(upset, score_upset)
...
final_upset = np.zeros([len(compare_names_all), args.num_trials*len(args.seeds), 2])
final_upset[:] = np.nan
...
new_shape_upset = (args.num_trials*len(args.seeds), len(compare_names_all), 2)
metric_names = ['test kendall tau', 'test kendall p', 'val kendall tau', 'val kendall p', \
'all kendall tau', 'all kendall p', 'upset_simple', 'upset_ratio']
